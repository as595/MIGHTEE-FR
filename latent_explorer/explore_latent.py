...
def main():
    ...
    st.set_page_config(layout="wide", page_title="MIGHTEE (RGZ Encoding)")
...
def tell_me_more():
    ...
    The galaxies are drawn from the MeerKAT MIGHTEE COSMOS Early Science data. Only galaxies with angular extent above 25 arcsec are shown.
...
def convert_df(df):
    ...
    return df[["mightee_name", "ra", "dec"]].to_csv(index=False).encode("utf-8")


def show_latent_space_interface(df):

    fig, ax = plot_latent_space(df)
    ...
    if x_y_dict is None:  # no click yet, return None
        ...
        offset = 6.432 # this value depends on whether axes are shown or not...
...
def plot_latent_space(df, figsize=(5.6, 5.6), data_pad=0.1):
    fig, ax = plt.subplots(figsize=figsize)
    ax.scatter(df["umap_x"], df["umap_y"], s=0.55, alpha=0.1)
    if st.session_state["x"] is not None:
        ax.scatter(st.session_state["x"], st.session_state["y"], marker="+", c="r")
...
def remove_duplicates(df, n, threshold=0.1):
    ...
    while i < n:
        ...
        if np.any(diff):
            ...
            print(f"Removing duplicate {df.iloc[i]['mightee_name']} and {df.iloc[idx_dup[0]]['mightee_name']}")
...
#    df = pd.read_parquet("umap.parquet")
    df = pd.read_parquet("mightee_umap.parquet")
#    plot_umap(df)
    return df

def plot_umap(df):
    fig, ax = plot_latent_space(df, None, None)
    fig.tight_layout()
    fig.savefig("umap.jpg", format="jpg", dpi=100)
    return
