...
def main():
    ...
    # load the RGZ model:
    model = BYOL.load_from_checkpoint("byol.ckpt", map_location=torch.device("cpu"))
    ...
    project_name = "BYOL_finetune_cpu"

    # put the seed in the config and seed everything:
    ...
    config["trainer"]["accelerator"] = "cpu" # for CPU only
    ...
    ft_checkpoint, model = run_finetuning(config, model.encoder, finetune_datamodule, logger)
    wandb.save(ft_checkpoint.best_model_path)
